...
4.进程池:任务多,任务小,需要多进程完成,用进程池避免进程创建消耗
    pool()
    apply_async()
    close()
    join()
...
****************************************************************
1.共享内存
    【1】通信原理:在内存中开辟一块空间，进程可以写入内容和读取内容
        内容完成通信,但是每次写入内容会覆盖之前内容。
    【2】实现方法
        ...
        参数:ctype 表示共享内存数据类型
        　　 data  整数则表示开辟空间的大小,
        　　       其他数据类型(列表,字符串))表示开辟空间存放的初始化数据
        返回值:共享内存对象

        Array共享内存读写:通过遍历obj可以得到每个值,直接可以通过索引下标修改任意值
...
四.python线程的GIL问题 （全局解释器锁）
	【1】什么是GIL:由于python解释器设计中加入了解释器锁,
     　　导致python解释器同一时刻只能解释执行一个线程,
     　　大大降低了线程的执行效率.	
	【2】导致后果:因为遇到阻塞时线程会主动让出解释器,去解释其他线程.
