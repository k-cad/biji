s一.基于threading的多线程网络并发
    【1】实现原理:每当一个客户端发起请求，就创建一个新的
    			线程处理客户端具体请求，主线程循环等待处
                理其他客户端连接。
	【2】实现步骤
			1. 创建监听套接字
			2. 循环接收客户端连接请求
...
二.集成模块完成多进程/线程网络并发
	...
	3.使用步骤
		1.创建服务器类，通过选择继承的类，决定创建TCP或者UDP，
          多进程或者多线程的并发服务器模型。
		2.创建请求处理类，根据服务类型选择stream处理类还是
          Datagram处理类。重写handle方法，做具体请求处理。
...
四.协程技术
  　1.定义：纤程(微线程)
    	　 是为非抢占式多任务产生子程序的计算机组件.
		 　协程允许不同入口点在不同位置暂停或开始，
		　 简单来说，协程就是可以暂停执行的函数.
		* yield可以实现上述函数暂停执行的功能，
		  即yield是python内部实现协程的基础关键字
		* 协程的本质其实是一个单线程程序，不会使用计算机多核资源,
		  但是可以在应用程序形成多个函数任务都被执行的状态,以处理多个任务
	2.协程原理：记录一个函数的上下文栈帧，协程调度切换时
			　　会将记录的上下文保存，在切换回来时进行调取，
